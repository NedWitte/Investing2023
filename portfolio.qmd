...
growth_rate <- 0.03  # 3% expressed as a decimal
additional_amount <- 10000

# Calculate the values for each year
values <- initial_amount * (1 + growth_rate)^years + additional_amount * years

# Create a dataframe
data <- data.frame(Year = years, Value = values)

# Create the plot using Plotly
plot <- plot_ly(data, x = ~Year, y = ~Value, type = 'bar', mode = 'markers')%>% 
  layout(
